...
class ViTSelfAttention(BaseModule):
    ...
    def forward(
        ...
        output_size = (query.size(0), query.size(1), query.size(2), key.size(2)*self.world_size)
    
        # [batch_size, num_heads, sub_seq_len, head_size] -> [batch_size * num_heads, sub_seq_len, head_size]
        query = query.reshape(output_size[0]*output_size[1], query.size(2), -1)
        ...
        # return outputs
        attn_output = attn_output.transpose(0, 1).contiguous().reshape(sub_seq_length * batch_size, embed_dim)
        attn_output = self.out_proj(attn_output)
        attn_output = attn_output.reshape(batch_size, sub_seq_length, embed_dim)
        return attn_output

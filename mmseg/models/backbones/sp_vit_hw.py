...
class SPVisionTransformer(BaseModule):
    ...
    def _pos_embeding(self, patched_img, hw_shape, pos_embed):
        ...
        if x_len != pos_len:
            ...
            pos_embed = torch.chunk(pos_embed, sp_h, dim=-2)[ind_h] if sp_h > 1 else pos_embed
            pos_embed = torch.chunk(pos_embed, sp_w, dim=-1)[ind_w] if sp_w > 1 else pos_embed
            # pos_embed = torch.chunk( torch.chunk(pos_embed, sp_h, dim=-2)[ind_h], sp_w, dim=-1 )[ind_w]
